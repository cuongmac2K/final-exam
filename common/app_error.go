...
func MyErr(statusCode int, root error, msg, key, log string) *AppError {
	return &AppError{
		StatusCode: statusCode,
		RootErr:    root,
		Message:    msg,
...
func ErrCannotGetEntity(entity string, err error) *AppError {
	return MyErr(
		404,
		err,
		fmt.Sprintf("id is not correct"),
		fmt.Sprintf("ErrCannotGet%s", entity),
		fmt.Sprintf(" Cannot get %v", strings.ToLower(entity)),
	)
}
...
func ErrCannotListUserTrendEntity(entity string, err error) *AppError {
	return NewCustomError(
		err,
		fmt.Sprintf("Cannot list user trend %s", strings.ToLower(entity)),
		fmt.Sprintf("ErrCannotList  user trend%s", entity),
	)
}
...
func ErrEmailNotRightFormat(err error) *AppError {
	return NewCustomError(
		err,
		fmt.Sprintf("This email address is not true"),
		fmt.Sprintf("ErrEmailNotRightFormat"),
	)
}
